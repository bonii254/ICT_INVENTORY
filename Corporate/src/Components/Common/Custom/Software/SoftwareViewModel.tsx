...
interface SoftwareViewModalProps {
  isOpen: boolean;
  toggle: () => void;
  software: Record<string, any> | null;
}

const SoftwareViewModal: React.FC<SoftwareViewModalProps> = ({
  isOpen,
  toggle,
  software,
}) => {
  ...
  const handlePrint = () => {
    ...
    if (printWindow && printableRef.current) {
      printWindow.document.write(
        "<html><head><title>Software Details</title></head><body>",
      );
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    const rows = Object.entries(software || {}).map(([key, value]) => [
    ...
    doc.save("software_details.pdf");
  };

  return (
    <Modal isOpen={isOpen} toggle={toggle} size="lg" centered>
      <ModalHeader toggle={toggle}>Software Details</ModalHeader>
      <ModalBody>
...
export default SoftwareViewModal;
