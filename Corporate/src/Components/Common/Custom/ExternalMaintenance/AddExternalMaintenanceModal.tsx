...
import AddProviderModal, {Provider} from '../Provider/AddProviderModel' 
...
const AddExternalMaintenanceModal: React.FC<AddExternalMaintenanceModalProps> = ({
  ...
  const { assets, providers, refetch } = useAssetOptions();
  ...
  const handleProviderCreated = (newProvider: Provider) => {
    if (newProvider.id === undefined) {
      toast.error('❌ Provider ID is missing. Cannot select provider.');
      return;
    }
    refetch?.();
    form.setFieldValue('provider_id', newProvider.id);
    toast.success(`✅ Provider "${newProvider.name}" added and selected!`);
  }; 
                ...
                {(field) => (
                  <Col md="6">
                    <Label className="form-label d-flex justify-content-between">
                      ...
                      <Button color="link" size="sm" onClick={() => setProviderModalOpen(true)}>
                        + Add Provider
                      </Button>
                    </Label>
                    <AsyncSelectInput
